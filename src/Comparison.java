...
//        System.out.println(bot2BestHand);
    ...
    private String compare() {
        ...
        for (int i = 0; i < this.bot1.evaluator.getHand().getBestFiveCards().size(); i++) {
            int bot1Rank = this.bot1.evaluator.getHand().getBestFiveCards().get(i).rank.ordinal();
            int bot2Rank = this.bot2.evaluator.getHand().getBestFiveCards().get(i).rank.ordinal();
            if ( bot1Rank > bot2Rank ) {
                return win1;
            } else if( i == this.bot1.evaluator.getHand().getBestFiveCards().size()-1 && bot1Rank == bot2Rank) {
                return draw;
            }
        }
        return win2;
